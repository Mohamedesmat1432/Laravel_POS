                            ...
                            <table class="table table-hover table-bordered">
                                ...
                                @foreach ($all_stores as $store)
                                    <tr>
                                        ...
                                        <td>
                                            <button href="" id="store-{{ $store->id }}" data-name="{{ $store->name }}"
                                                data-id="{{ $store->id }}" data-product_id="{{ $product->id }}"
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody class="order-list">
                                        ...
                                        @foreach ($products as $store)
                                            <tr>
                                                <td>{{ $store->name }}</td>
                                                <td><input type="number" name="stores[{{ $store->id }}][first_stock]"
                                                        data-first_stock="{{ number_format($store->first_stock, 2) }}"
                                                        class="form-control input-sm first_stock" min="0"
                                                        value="{{ $store->pivot->first_stock }}"></td>
                                                <td><input type="number" name="stores[{{ $store->id }}][stock]"
                                                        data-stock="{{ number_format($store->stock, 2) }}"
                                                        class="form-control input-sm stock" min="0"
                                                        value="{{ $store->pivot->stock }}"></td>
                                                <td><input type="number" name="stores[{{ $store->id }}][product_id]"
                                                        data-stock="{{ number_format($store->product_id, 2) }}"
                                                        class="form-control input-sm product_id hidden" min="0"
                                                        value="{{ $store->pivot->product_id }}"></td>
                                                ...
                                                <td>
                                                    <button class="btn btn-danger btn-sm remove-store-btn"
