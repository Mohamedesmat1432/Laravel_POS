...
class OrderSuppliersController extends Controller
{
    public function index(Request $request)
    {
        $orders_suppliers = OrderSupplier::whereHas('supplier', function ($q) use ($request) {

            return $q->whereTranslationLike('name', '%' . $request->search . '%');

        })->paginate(10);

        return view('dashboard.ordersuppliers.index', compact('orders_suppliers'));

    }//end of index

    public function products(OrderSupplier $order)
    {
      //  dd($order);
        $products = $order->products;
        return view('dashboard.ordersuppliers._products', compact('order', 'products'));
    ...
    public function destroy(OrderSupplier $orders_supplier)
    {
       //dd($orders_supplier);
