...
class OrdersSuppliersController extends Controller
{
    public function create(Supplier $supplier, Request $request)
    {
        ...
        $orders = $supplier->orders()->with('products')->paginate(25);
        return view('dashboard.suppliers.orders.create', compact('supplier', 'categories', 'orders', 'products', 'stores', 'mony_stocks'));
    } //end of create
    ...
    public function store(Request $request, Supplier $supplier)
    {
        ...
        return redirect()->route('dashboard.orders_suppliers.index');
    } //end of store

    public function edit(Supplier $supplier, OrderSupplier $order)
    {
        $stores = Store::all();
        $mony_stocks = MonyStock::all();
        ...
        $categories = Category::with('products')->get();
        $orders = $supplier->orders()->with('products')->paginate(25);
        return view('dashboard.suppliers.orders.edit', compact('supplier', 'order', 'categories', 'orders', 'stores', 'mony_stocks'));
    } //end of edit

    public function update(Request $request, Supplier $supplier, OrderSupplier $order)
    {
        $request->validate([
            'products' => 'required|array',
        ]);
